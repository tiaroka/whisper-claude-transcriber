...
def process_files(input_directory, temp_directory, max_workers=None):
    """指定されたディレクトリ内のファイルを並列処理し、一時ディレクトリに出力する。"""
    ...
    def process_file(file_path):
        ...
        success = extract_audio(str(file_path), str(output_file), None)
        return {
            "file_path": file_path,
            "success": success
        }
    ...
    print(f"並列処理を開始します（最大ワーカー数: {max_workers or 'CPU数'}）")
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 全てのファイルを並列処理
        future_to_file = {executor.submit(process_file, file_path): file_path for file_path in files_to_process}
        
        # tqdmで進捗バーを表示しながら結果を処理
        with tqdm(total=len(files_to_process), desc="Overall Progress") as pbar:
            for future in concurrent.futures.as_completed(future_to_file):
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        error_count += 1
                        pbar.write(f"✗ Failed: {file_path.name}")
                except Exception as e:
                    error_count += 1
...
def process_audio_files(temp_directory, segment_length_ms, overlap_ms, max_workers=None):
    """一時ディレクトリ内のエンコードされたオーディオファイルを並列処理する。"""
    # 処理対象ファイルのリストを作成
    files_to_process = [
    ...
    if not files_to_process:
        print("処理対象のエンコード済みファイルが見つかりませんでした。")
        return
    ...
    def process_file(input_file):
        try:
            ...
            return {"file": input_file, "success": True}
        except Exception as e:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        # tqdmで進捗バーを表示しながら結果を処理
        with tqdm(total=len(files_to_process), desc="Splitting Audio Files") as pbar:
            for future in concurrent.futures.as_completed(future_to_file):
                file_path = future_to_file[future]
                try:
                    result = future.result()
                    if result["success"]:
                        pbar.write(f"✓ Split completed: {os.path.basename(file_path)}")
                    else:
                        pbar.write(f"✗ Split failed for {os.path.basename(file_path)}: {result.get('error', 'Unknown error')}")
                except Exception as e:
                    pbar.write(f"✗ Error processing {os.path.basename(file_path)}: {str(e)}")
                
                pbar.update(1)
    
    print("全ての音声分割処理が完了しました。")
...
class AudioTranscriber:
    ...
    def process_files(self, max_workers=None):
        ...
        if not target_files:
            print(f"処理対象のファイルが見つかりませんでした。プレフィックス: {self.output_prefix}")
            return
        
        print(f"文字起こし処理を開始します（最大ワーカー数: {max_workers or 'CPU数'}）")
        
        # 並列処理用の関数
        def process_file_task(fname):
            output_txt_file = os.path.join(self.output_temp_directory, fname.replace(".mp3", ".txt"))
            
            if os.path.exists(output_txt_file):
                return {"file": fname, "status": "skipped", "message": "File already exists"}
            
            full_path = os.path.join(self.input_temp_directory, fname)
            
            try:
                self.transcribe_audio(full_path, output_txt_file)
                return {"file": fname, "status": "success", "output": output_txt_file}
            except Exception as e:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 全てのファイルを並列処理
            future_to_file = {executor.submit(process_file_task, fname): fname for fname in target_files}
            
            # tqdmで進捗バーを表示しながら結果を処理
            with tqdm(total=len(target_files), desc="Transcribing Audio Files") as pbar:
                for future in concurrent.futures.as_completed(future_to_file):
                    ...
                    try:
                        ...
                        if status == "success":
                            pbar.write(f"✓ Transcribed: {fname} -> {os.path.basename(result['output'])}")
                        elif status == "skipped":
                            pbar.write(f"- Skipped: {fname} ({result['message']})")
                        else:
                            pbar.write(f"✗ Failed: {fname} ({result['message']})")
                    except Exception as e:
                        pbar.write(f"✗ Error processing {fname}: {str(e)}")
                    
                    pbar.update(1)
        
        print("全ての文字起こし処理が完了しました。")
    ...
    def transcribe_audio(self, audio_file_path, output_txt_file):
        ...
        with open(audio_file_path, "rb") as audio_file:
            response = self.client.audio.transcriptions.create(
                ...
                response_format="vtt",  # text, jsonも指定可能 Whisper-1 の場合 vttも選択可能
...
def process_files_with_claude(input_directory, output_directory, output_prefix, max_workers=None):
    """Claudeを使用して複数のファイルを並列処理します。"""
    ...
    print(f"ディレクトリ内のファイル数: {len(files_in_directory)}")
    ...
    for filename in files_in_directory:
        if file_pattern.match(filename):
            ...
            target_files.append((filename, input_path, output_path))
        else:
    ...
    def process_file(file_info):
        filename, input_path, output_path = file_info
        ...
        try:
            ...
            return {"file": filename, "status": "success", "path": output_path}
            
        except anthropic.APIError as e:
            if e.status_code == 529:
                return {"file": filename, "status": "overloaded", "path": input_path, "error": str(e)}
            else:
                return {"file": filename, "status": "api_error", "path": input_path, "error": str(e)}
        except Exception as e:
            return {"file": filename, "status": "error", "path": input_path, "error": str(e)}
    
    print(f"Claudeによる文字起こし処理を開始します（最大ワーカー数: {max_workers or 'CPU数'}）")
    
    # ThreadPoolExecutorを使用して並列処理
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 全てのファイルを並列処理
        future_to_file = {executor.submit(process_file, file_info): file_info for file_info in target_files}
        
        # tqdmで進捗バーを表示しながら結果を処理
        with tqdm(total=len(target_files), desc="Processing with Claude") as pbar:
            for future in concurrent.futures.as_completed(future_to_file):
                ...
                try:
                    ...
                    if status == "success":
                        pbar.write(f"✓ 処理完了: {filename}")
                    elif status == "overloaded":
                        pbar.write(f"! 過負荷エラー: {filename} - 後でリトライします")
                        failed_files.append(result["path"])
                    else:
                        pbar.write(f"✗ エラー: {filename} - {result.get('error', '不明なエラー')}")
                except Exception as e:
                    pbar.write(f"✗ 予期せぬエラー: {filename} - {str(e)}")
...
def run_audio_conversion(max_workers=None):
    ...
    try:
        ...
        # ファイルの処理（並列処理）
        results = process_files(base_input_dir, temp_directory, max_workers)
        print("\n全てのファイルの処理が完了しました。")
        ...
        process_audio_files(temp_directory, segment_length_ms, overlap_ms, max_workers)
        print("全ての分割ファイルの処理が完了しました。")
        
    except Exception as e:
        print(f"処理中にエラーが発生しました: {str(e)}")

def run_transcription(max_workers=None):
    ...
    try:
        ...
        transcriber.process_files(max_workers)
...
def run_post_processing(max_workers=None):
    ...
    try:
        ...
        # Claudeによる文字起こし後処理（並列処理）
        failed_files = process_files_with_claude(latest_temp_dir, base_output_dir, output_prefix, max_workers)
...
def main():
    ...
    try:
        ...
        print(f"システムのCPUコア数: {default_workers}")
    except:
        default_workers = None
        print("システムのCPUコア数を取得できませんでした。")
    
    max_workers_input = input(f"並列処理のワーカー数を入力してください（デフォルト: {default_workers or 'CPU数'}）: ")
    if max_workers_input.strip():
        try:
            max_workers = int(max_workers_input)
            print(f"ワーカー数を {max_workers} に設定しました。")
        except ValueError:
            max_workers = default_workers
            print(f"無効な入力です。ワーカー数をデフォルト値 {default_workers or 'CPU数'} に設定します。")
    else:
        max_workers = default_workers
        print(f"ワーカー数をデフォルト値 {default_workers or 'CPU数'} に設定します。")
    ...
    while True:
        ...
        if choice == "1":
            run_audio_conversion(max_workers)
        elif choice == "2":
            run_transcription(max_workers)
        elif choice == "3":
            run_post_processing(max_workers)
        elif choice == "4":
